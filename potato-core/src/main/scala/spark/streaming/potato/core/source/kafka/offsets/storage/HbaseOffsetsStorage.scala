...
import spark.streaming.potato.core.source.kafka.KafkaSourceConstants
  ...
  override def save(groupId: String, offsets: Map[TopicAndPartition, Long]): Boolean = {
    ...
    val puts = offsets.groupBy(_._1.topic).map { ttapo =>
      ...
      ttapo._2.foreach { tapo =>
        put.addColumn(
          Bytes.toBytes(conf.getOrElse(HbaseOffsetsStorage.HBASE_FAMILY_KEY, HbaseOffsetsStorage.HBASE_FAMILY_DEFAULT)),
...
object HbaseOffsetsStorage {
  val HBASE_OFFSETS_STORAGE_PREFIX: String = KafkaSourceConstants.KAFKA_SOURCE_PREFIX + "offsets.storage.hbase."
