...
import spark.potato.hbase.util.TableUtil._
...
class TableUtilTest {
  ...
  def withBufferedSinkTableTest(): Unit = {
    for (i <- 0 until 4) {
      withBufferedSinkTable(conf, "test") { table =>
        for (j <- 0 until 2000) {
          table.sink(new Put(Bytes.toBytes(s"$i-$j")).addColumn(
            Bytes.toBytes("f1"),
            Bytes.toBytes("test"),
            Bytes.toBytes("hello")
          ))
          if (j % 2 == 0)
            table.sink(new Delete(Bytes.toBytes(s"$i-$j")).addColumns(
              Bytes.toBytes("f1"),
              Bytes.toBytes("test")
            ))
        }
  ...
  def withSynchronizedBufferedSinkTableTest(): Unit = {
    withBufferedSinkTable(conf, "test", synchronized = true) { table =>
      class Runner(id: Int) extends Runnable {
        override def run(): Unit = {
          for (i <- 0 until 1000) {
            ...
            table.sink(new Put(key).addColumn(family, column, value))
            if (i % 2 == 0)
              table.sink(new Delete(key).addColumns(family, column))
          }
        }
      }
