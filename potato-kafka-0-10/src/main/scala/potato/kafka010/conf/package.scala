...
package object conf {
  ...
  val POTATO_KAFKA_PRODUCER_FAILED_RATIO_KEY: String = POTATO_KAFKA_PRODUCER_PREFIX + "failed.ratio"
  val POTATO_KAFKA_PRODUCER_FAILED_RATIO_DEFAULT: String = 0.toString // 默认出现一次失败即认为该批次失败。
  val POTATO_KAFKA_PRODUCER_FAILED_RATIO_BACKOFF_KEY: String = POTATO_KAFKA_PRODUCER_PREFIX + "failed.ratio.backoff"
  val POTATO_KAFKA_PRODUCER_FAILED_RATIO_BACKOFF_DEFAULT: String = 10.toString // 在处理10条记录后，或者在处理条数大于POTATO_KAFKA_PRODUCER_FAILED_RECORD_KEY时开始计算失败比率。
  val POTATO_KAFKA_PRODUCER_FAILED_RECORD_KEY: String = POTATO_KAFKA_PRODUCER_PREFIX + "failed.record"
  val POTATO_KAFKA_PRODUCER_FAILED_RECORD_DEFAULT: String = 0.toString // 默认出现一次失败即认为该批次失败。
  val POTATO_KAFKA_PRODUCER_SPEED_LIMIT_KEY: String = POTATO_KAFKA_PRODUCER_PREFIX + "speed.limit"
