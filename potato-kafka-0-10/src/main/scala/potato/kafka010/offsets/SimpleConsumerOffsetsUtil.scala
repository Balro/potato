 ...
 *
 * @deprecated 新版本kafka建议使用[[KafkaConsumerOffsetsUtil]]
 */
object SimpleConsumerOffsetsUtil extends Logging {
          ...
          case o if o < earliest(offset._1) || o > latest(offset._1) =>
            if (reset) {
              config.autoOffsetReset match {
                case OffsetRequest.SmallestTimeString | "earliest" => earliest(offset._1)
                case OffsetRequest.LargestTimeString | "latest" => latest(offset._1)
 ...
 * offsets工具类隐式转换。
 */
object SimpleConsumerOffsetsUtilImplicits {
  /**
  ...
  implicit def mapToBrokerEndPoints(map: Map[String, Int]): Set[BrokerEndPoint] = {
    map.map { m =>
      BrokerEndPoint(SimpleConsumerOffsetsUtil.unknownBrokerId, m._1, m._2)
    }.toSet
  }
  ...
  implicit def mapToConsumerConfig(map: Map[String, String], addDefault: Boolean = true): ConsumerConfig = {
    ...
    props ++= map
    props.get("auto.offset.reset") match {
      case "earliest" => props.setProperty("auto.offset.reset", OffsetRequest.SmallestTimeString)
      case "latest" => props.setProperty("auto.offset.reset", OffsetRequest.LargestTimeString)
      case _ =>
    }
    new ConsumerConfig(props)
  }
