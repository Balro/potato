 ...
 *
 * 修改者：Balro
 * 修改位置：writeFields() 添加写json时写null值功能。
 */
class PotatoJacksonGenerator(
                                             dataType: DataType,
                                             writer: Writer,
                                             options: PotatoJSONOptions) {
  ...
  private def writeFields(
    ...
    while (i < row.numFields) {
      ...
      if (!row.isNullAt(i)) {
        ...
        gen.writeNullField(field.name) // 添加写null值功能。 @balro
      }
      i += 1
    }
  }
